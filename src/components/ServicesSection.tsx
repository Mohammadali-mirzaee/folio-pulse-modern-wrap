...
const ServicesSection = () => {
  ...
  const services: Service[] = [
    ...
    { 
      icon: PaintRoller, 
      title: "Chrome Delete", 
      description: "Omvandla blanka kromdetaljer till elegant matt eller svart finish." 
    },
    { 
      icon: Droplet, 
      title: "Solfilm", 
      description: "UV-skydd och värmereducering med högkvalitativ fönsterfilm." 
    },
    { 
      icon: Film, 
    ...
    { 
      icon: Palette, 
      title: "Styling", 
      description: "Skräddarsydda designlösningar för att framhäva din bils karaktär." 
    },
  ];
      ...
      <div className="container-wide">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
          {services.map((service, index) => (
            ...
            >
              <div 
                className={`bg-accent/10 p-6 rounded-full mb-6 transition-all duration-300 ${hoveredService === index ? 'bg-accent/30 scale-110' : ''}`}
              >
                <service.icon 
                  size={36} 
                  className={`text-accent transition-all duration-300 ${hoveredService === index ? 'animate-pulse' : ''}`} 
                />
              </div>
              <h3 className="text-xl md:text-2xl font-poppins font-semibold text-center mb-2">
                {service.title}
              </h3>
              <p className={`text-center text-white/60 transition-all duration-300 ${hoveredService === index ? 'text-white/90' : 'text-white/60'}`}>
                {service.description}
              </p>
            </div>
          ))}
